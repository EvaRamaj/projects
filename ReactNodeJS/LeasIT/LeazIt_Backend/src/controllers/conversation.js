        ...
        .exec(function(err, conversations) {
            ...
            conversations.forEach(function(conversation) {
                MessageModel.find({ 'conversationId': conversation._id })
                    ...
                    .populate({
                        path: "author",
                        select: 'username photo' // TODO -- firstname? username?
...
const getConversation = (req, res, next) => {
    ConversationModel.findOne({_id: req.params.conversationId})
    // .select('_id')
        .exec(function (err, conversation) {
            ...
            MessageModel.find({conversationId: conversation._id})
                .sort({'createdAt': 'asc'})    //sort does not work correctly in postman but works on db
                .populate({
                    path: "author",
                ...
                .exec(function (err, message) {
                    ...
                    return res.status(200).json({conversations: fullConversation});
                });
        });
};
...
const sendReply = (req, res, next) =>{
    ...
    // .select('_id')
        .exec(function (err, conversation) {
            ...
            if(conversation.participants[0] == req.userId){
